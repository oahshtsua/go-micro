...
func (m *Mail) sendSMTPMessage(msg message) error {
    ...
    log.Println("Getting encryption:")
    log.Println(m.getEncryption(m.Encryption))
	mailSrv := mail.NewSMTPClient()
	mailSrv.Host = m.Host
	mailSrv.Port = m.Port
	mailSrv.Username = m.Username
	mailSrv.Password = m.Password
	mailSrv.Encryption = m.getEncryption(m.Encryption)
    ...
    log.Println("Here after mailSrv connect", err)
	if err != nil {
		log.Println(err)
		return err
	}
